...
def process_tfr(path, data_dir, test_flag=False):
    """
...
def download_and_process(filename, data_dir):
    ...
    local_path = os.path.join("data/waymo/animation_input", filename)
    process_tfr(local_path, data_dir, test_flag=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', required=True,
                        help='data directory')
    #parser.add_argument('--size', required=False, default=75, type=int,
    ...
    # init ray
    filenames = os.listdir("data/waymo/animation_input")
    ray.init(num_cpus=cpu_count())
    workers = [download_and_process.remote(fn, data_dir) for fn in filenames]
    _ = ray.get(workers)
